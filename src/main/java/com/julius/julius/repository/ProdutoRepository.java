...
public interface ProdutoRepository extends JpaRepository<Produto, Long> {
        ...
        @Query(value = "SELECT p.* FROM produtos p JOIN produto_link pl ON p.produto_id = pl.produto_id JOIN links_produtos lp ON pl.link_produto_id = lp.id WHERE lp.site = :site AND LOWER(p.titulo) LIKE LOWER(CONCAT('%', :termoPesquisa, '%'))", nativeQuery = true)
